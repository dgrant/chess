...
mod alpha_beta_tests {
    ...
    fn test_alpha_beta_pruning_consistency() {
        ...
        let (best_move_ab, _score) = Searcher::new().find_best_move(&mut board, 3);
    ...
    fn test_alpha_beta_tactical_position() {
        ...
        let (best_move, _score) = Searcher::new().find_best_move(&mut board, 4);
        assert!(
            best_move.is_some(),
    ...
    fn test_alpha_beta_checkmate_detection() {
        ...
        let (best_move, _score) = Searcher::new().find_best_move(&mut board, 2);
